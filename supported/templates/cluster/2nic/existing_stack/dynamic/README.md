...
```bash
openstack stack create stack-cluster-2nic-test -t src/f5-openstack-hot/supported/templates/cluster/2nic/existing_stack/dynamic/f5_bigip_cluster_2_nic.yaml -e src/f5-openstack-hot/supported/templates/cluster/2nic/existing_stack/dynamic/f5_bigip_cluster_2_nic_env.yaml
```
...
| Parameter | Required | Description | Constraints |
| --- | :---: | --- | --- |
| bigip_licensing_type | Yes |  Type of licensing to use for BIG-IP. |  Allowed values are `BYOL` (License Key), or `NO_LIC` (skip licensing). |
...
| bigip_self_port_lockdown | No | Optional list of service:port lockdown settings for the VLAN. If no value is supplied, default is used.  |  Syntax: List of `service:port` example: `[tcp:443, tcp:22]`. Additional allowed values: `[allow-all, allow-default, allow-none]` |
