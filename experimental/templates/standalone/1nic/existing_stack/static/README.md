...
- The management interface IP address is configured statically.
...
```bash
openstack stack create stack-1nic-test -t src/f5-openstack-hot/experimental/templates/standalone/1nic/existing_stack/static/f5_bigip_standalone_1_nic.yaml -e src/f5-openstack-hot/experimental/templates/standalone/1nic/existing_stack/static/f5_bigip_standalone_1_nic_env.yaml
```
...
| Parameter | Required | Description | Constraints |
| --- | :---: | --- | --- |
| bigip_licensing_type | Yes |  Type of licensing to use for BIG-IP. |  Allowed values are `BYOL` (License Key), or `NO_LIC` (skip licensing) or `BIGIQ` (via BIG-IQ License Manager v5.2 or v5.3). |
