...
```bash
openstack stack create stack-1nic-test -t src/f5-openstack-hot/experimental/templates/standalone/1nic/prod_stack/static/f5_bigip_standalone_1_nic.yaml -e src/f5-openstack-hot/experimental/templates/standalone/1nic/prod_stack/static/f5_bigip_standalone_1_nic_env.yaml
```
...
| Parameter | Required | Description | Constraints |
| --- | :---: | --- | --- |
| bigip_licensing_type | Yes |  Type of licensing to use for BIG-IP. |  Allowed values are `BYOL` (License Key), `BIGIQ` (via BIG-IQ License Manager), or `NO_LIC` (skip licensing). |
