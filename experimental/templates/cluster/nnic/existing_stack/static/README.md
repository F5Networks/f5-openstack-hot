...
- Management interface IP is configured statically. You must still have an IP allocation pool configured on the subnet in order to assign a fixed IP address to the instance.
...
```bash
openstack stack create stack-nnic-test -t src/f5-openstack-hot/experimental/templates/cluster/nnic/existing_stack/static/f5_bigip_cluster_n_nic.yaml -e src/f5-openstack-hot/experimental/templates/cluster/nnic/existing_stack/static/f5_bigip_cluster_n_nic_env.yaml
```
